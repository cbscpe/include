...
;
;
;	2023-06-18	PS	Version 3.0
;				New OSCALL logic
;				It does not work with miniDTU, so we need to create
;				small testenvironments to find the reason behind
;				the failure
;	2023-12-27	PS	New OSCALL logic works on ATmega4809
;	2023-12-27	PS	iotime is now used differently, this 16-bit
;				timer is now just incremented every TICK and
;				when bit9 of the counter overflows, i.e. when
;				it changes from 1->0 we know that 1024 ticks
;				have been passed and start io-queue processing.
;	2023-12-27	PS	We use PIT of the RTC to generate the tick
;				interrupt, this saves a timer, and we now use
;				iotime as the timestamp for test output, which
;				saves another timer.
;				This is also the reason we new check for overflow
;				of bit9 and just increment iotime every tick.
;				The disadvantage is that ticks are now 1/1024th
;				of a second and the timestamp no longer counts
;				from 0 to 59999 but to 65535. This is because
;				RTC uses the internal 32768Hz clock and we use
;				a divider of 32 to generate the PIT interrupt
;				A iotime cycle now takes 64seconds and is no 
;				longer exactly a minute.
;
;--------------------------------------------------------------------------
	.cseg
;
;	Local Macro to wrap the software interrupt. We use a software 
;	interrupt for the OS calls. This is done via a level triggered
;	PIN that we can clear in software. 
;
;	Version 3.0 new logic using ISR
;
;	When a job calls a RT-OS function we will now clear the b_RTOS
;	bit and wait until it is set by the ISR. The ISR no just sets
;	the b_RTOS bit again and then executes a normal 'ret' instruction
;	therefore we return to the code that waits for b_RTOS being set
;	in level 0 interrupt mode. Now we just proceed with saving the
;	processor status and prepare the stack and then acknowledge the
;	interrupt and jump to the real code for the RT-OS function.
;
;	The advantage is that we now preserve the Z register.
;
;	t.b.d.: change the OS routines so we can omit the local caller
;
;	2023-12-27	On the ATmega4809 this works without the local
;			caller. We need to verify this with the AVR128Dx
;			processor family, there should not be any difference
;
.macro	oscall
	...
	rjmp	PC-1			; 
	push	zh			; After ISR executed and before
	push	zl			; acknowledging the interrupt
	push	yh			; we need to let pass some (>1)
	push	yl			; cycles
	sbi	f_RTOS			; Acknowledge Pin Change Interrupt
	rjmp	@0			; Go to real routine
.endmacro
;--------------------------------------------------------------------------
;
;	We use a pin-change interrupt with LEVEL 0 to enter the OS routines.
...
rtos_:
	...
	rjmp	rtos_010
	sbi	b_RTOS			; Acknowledge OS Call Request
	ret

rtos_010:
	ldi	r16, crash_spurious	; this must never happen
	call	crash			; crashdump (at the moment simple crash)
;
...
tick:
	...
	lds	zl, iotime+0
	lds	zh, iotime+1
	mov	yh, zh			; Remember State of Bit9
	adiw	zh:zl, 1
	sts	iotime+0, zl
	sts	iotime+1, zh
	brne	tick100
	lds	xl, iotime+2		; Have an internal 32-bit timestamp
	lds	xh, iotime+3
	adiw	xh:xl, 1
	sts	iotime+2, xl
	sts	iotime+3, xh
;
;	Instead of iotime to count down from 1000 to 0 we now use
;	the transition of bit9 from high to low when just counting
;	the 16-bit iotime value up. With this we can use iotime as
;	the ticker count. When using the PIT of the RTC with a pre
;	scale divider of 32 this will happen exactly every second.
;
tick100:
	com	zh			; Complement of new Bit9
	and	zh, yh			; old-bit9^¬new-bit9
	sbrs	zh, 1			; Bit 9 was set and now is cleared -> 1024 ovr
	rjmp	tick150			; No need to check IO time-outs
tick110:
...
resume030:				; attempt to resume a non-suspended ioq
	ldi	xl, chk_resume+0x02	; we should actually call the crash handler
	rjmp	resume060		;
resume040:
...
resume060:
	...
	sts	tesoutptr+1, yh
resume070:
	ldi	xl, chk_invalid		; Invalidate the saved tesoutent
	sts	tesoutent+0, xl
	pop	xl
	pop	xh
	pop	yl
...
;
	st	-Y, r23			; New: the start parameter of the job
	st	-Y, r22			; created is passed as the second paremeter
	st	-Y, r23			; so we literally duplicate the value
	st	-Y, r22			; given by the caller of create
	st	-Y, r21
	st	-Y, r20
	st	-Y, r19
	st	-Y, r18
