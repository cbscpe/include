...
.macro	oscall
	...
	cbi	VPORTE_OUT, 0		; Trigger software interrupt
	sbis	VPORTE_OUT, 0		; check ISR has been executed
	rjmp	PC-1			; 
	ret				; Normal return
.endmacro
;--------------------------------------------------------------------------
...
;
rtos_:
	sbis	VPORTE_OUT, 0		; Is it an OSCALL software interrupt
	rjmp	oscall_			; yes ->
	sbis	VPORTE_OUT, 1		; Is it an ATTENTION software interrupt
	rjmp	attention_		; yes ->
...
oscall_:
	...
	in	r8, SREG
	sbi	VPORTE_OUT, 0		; important, between setting the pin and
	push	zh			; acknowledging the interrupt we need to
	push	zl			; have at least one additional cpu cycle!
	push	yh
	push	yl
	sbi	VPORTE_INTFLAGS, 0	; Acknowledge interrupt
	ijmp	
...
attention_:
	push	r8			; save minimal context
	in	r8, SREG
	sbi	VPORTE_OUT, 1		; important, between setting the pin and
	push	zh			; acknowledging the interrupt we need to
	push	zl			; have at least one additional cpu cycle!
	push	yh
	push	yl
	sbi	VPORTE_INTFLAGS, 1	; Acknowledge interrupt
	ldi	zl, low(intf)
	ldi	zh, high(intf)
	rjmp	unblocki
;--------------------------------------------------------------------------
...
sysext:	pop	yl			; unwind minimal context
	...
	pop	r8
	#ifdef dbg_sysrun
	cbi	VPORTD_OUT, dbg_sysrun
	#endif
	#ifdef dbg_sysret
	cbi	VPORTD_OUT, dbg_sysret
	#endif
	#ifdef dbg_tick
	cbi	VPORTD_OUT, dbg_tick
	#endif
	#ifdef dbg_block
	cbi	VPORTD_OUT, dbg_block
	#endif
	#ifdef dbg_unblock
	cbi	VPORTD_OUT, dbg_unblock
	#endif
	#ifdef dbg_release
	cbi	VPORTD_OUT, dbg_release
	#endif
	#ifdef dbg_acquire
	cbi	VPORTD_OUT, dbg_acquire
	#endif
	#ifdef dbg_null
	cbi	VPORTD_OUT, dbg_null
	#endif
	reti
;--------------------------------------------------------------------------
;
...
sysrun:	
	#ifdef dbg_sysrun
	sbi	VPORTD_OUT, dbg_sysrun
	#endif
	subi	yl, 0			;;; Test runjob
	sbci	yh, 0
	...
	out	SPL, r0
	out	SPH, r1			;;; Setup stack pointer of next context
...
sysnul:
	...
	#endif
	clr	r0
	sts	curjob+0, r0		;;; no current user job
	sts	curjob+1, r0
...
null:
	...
	sbi	VPORTD_OUT, dbg_sysnul
	#endif
	rjmp	null			; No job scheduled, so just loop
;--------------------------------------------------------------------------
;
...
tick:
	...
	subi	zl, 0
	sbci	zh, 0			;;; subtract 0 to test for 0
	...
	cbi	VPORTD_OUT, dbg_tick
#endif
	out	SREG, r8		;;; we always return from the timer interrupt
	pop	r8
	reti
tick100:
	push	yh
...
;	cpc	zh, xl
	subi	zl, 0
	sbci	zh, 0			;;; Check whether Z is zero (no more jobs)
	brne	tick110
	pop	xl
	pop	xh
	rjmp	sysret
...
link100:			; 100$:
	...
	ld	yh, X+			; mov	(r1)+, r2
	subi	yl, 0
	sbci	yh, 0			; subtract 0 to test for 0
...
release100:			; 100$:	
	...
	subi	xl, 0
	sbci	xh, 0			; subtract 0 to test for 0
...
;	indicating that the even has already taken place. Should a job then
...
unblocki:				;;; Entry for interrupt service routines
	#ifdef dbg_unblock
	sbi	VPORTD_OUT, dbg_unblock
	#endif
	ldd	yl, Z+0
	ldd	yh, Z+1			;;; Get block status
	sbiw	yh:yl, 1		;;; Has event already occured
	brne	unblock040		;;; No, signal event

	pop	yl			;;; we already have a pending unblock/event
	pop	yh
	pop	zl
	pop	zh
	out	SREG, r8
...
create_:	
	...
	st	-Y, r1			; we start with -Y as the pointer given 
	st	-Y, r8			; in the control block points past the
	st	-Y, zh			; stack area
	st	-Y, zl			; 
	st	-Y, r3			; The copy of the original Y register 
	st	-Y, r2			;
	st	-Y, xh
	st	-Y, xl
;	st	-Y, r25
;	st	-Y, r24			; replaced by value for ABI of job()
	...
	ldi	r16, CPU_I_bm		; Set I in process SREG
	st	-Y, r16			; Save it on stack on the 8th position
	pop	r16			; Resto
	st	-Y, r7
	st	-Y, r6
	st	-Y, r5
