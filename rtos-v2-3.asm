...
;	2023-01-13	PS	in conjunction with intsave we interoduce two
;				new OSCALLs: stop and unstop and unblocki will
; 				be most likely renamed to resume
;	2023-01-13	PS	waitqueue and sigqueue, documentation see below

;--------------------------------------------------------------------------
	.cseg
;
...
;--------------------------------------------------------------------------
;
;	We will add two new functions, waitqueue and sigqueue. This is to
;	implement a message passing interface. A queue in this context
;	is not a location but a control block, the queue control block.
;
;	The queue control block is 8 bytes.
;
;	.word	1	; Message queue head
;	.word	1	; Wait queue head
;	.word	1	; Queue queue head
;	.byte	1	; priority
;	.byte	1	; flags
;
;	Queue control blocks must be initialised with zero!
;
;	The first time a queue is used with waitqueue and no message is
;	in the message queue the queue itself is added to the queue queue
;	and the task is added to the wait queue of the queue control block.
;
;	With this `tick` will know of all queues with jobs potentially 
;	waiting. Tick will then go through all queues in the queue queue
;	and check for jobs waiting and decrement the wait timer. 
;
;	void* waitqueue(struct qcb* queue, uint16_t timeout);
;
;	In case of timeout = 0 the job will wait until a message has
;	been queued. Otherwise it will wait timeout ticks and after 
;	that it will be removed from the queue
;
;	void sigqueue(struct qcb* queue, void* message);
;
;	This will insert the message into the queue if the queue is not
;	empty. If the queue is empty it will check whether there is a
;	job waiting in this case it will remove the job from the wait
;	queue and insert it to the runjob and give the message to the job.
;
;--------------------------------------------------------------------------
;
